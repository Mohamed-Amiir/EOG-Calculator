...
class App:
    def __init__(self, root):
        ...
        GButton_560["text"] = "7"
        GButton_560.place(x=465,y=240-15,width=70,height=25)
        ...
        GButton_61["text"] = "8"
        GButton_61.place(x=465,y=290-15,width=70,height=25)
        ...
        GButton_383["text"] = "9"
        GButton_383.place(x=465,y=340-15,width=70,height=25)
    ...
    def move_cursor(self, event):
        key = event.keysym
        if key == "Up":
            if len(self.entry.get()) == 0 or len(self.entry.get()) == 2:
                if self.cursor_x == 290 and self.cursor_y == 275:
                    self.cursor_y = max(0, self.cursor_y - 100)
                elif self.cursor_x == 465 or self.cursor_x == 115:
                    self.cursor_y = min(529, self.cursor_y - 50)    
            else:
            ...
            if len(self.entry.get()) == 0 or len(self.entry.get()) == 2:
                if self.cursor_x == 290 and self.cursor_y == 275:
                    self.cursor_y = min(529, self.cursor_y + 100)
                elif self.cursor_x == 465 or self.cursor_x == 115:
                    self.cursor_y = min(529, self.cursor_y + 50)
            else:
            ...
            if len(self.entry.get()) == 0 or len(self.entry.get()) == 2:
                if self.cursor_x == 290 and self.cursor_y == 275:
                    self.cursor_x = max(0, self.cursor_x - 175)
                elif self.cursor_y == 175 or self.cursor_y == 375:
                    self.cursor_x = min(529, self.cursor_x - 75)    
            else:
                self.cursor_x = max(0, self.cursor_x - 75)

        elif key == "Right":
            if len(self.entry.get()) == 0 or len(self.entry.get()) == 2:
                if self.cursor_x == 290 and self.cursor_y == 275:
                    self.cursor_x = min(613, self.cursor_x + 175)
                elif self.cursor_y == 175 or self.cursor_y == 375:
                    self.cursor_x = min(529, self.cursor_x + 75)    
            else:
                self.cursor_x = min(613, self.cursor_x + 75)
    ...
    def is_cursor_over_button(self, button):
        ...
        return (button_x1 <= self.cursor_x <= button_x2 and button_y1 <= self.cursor_y <= button_y2) or (button_x1 <= self.cursor_x <= button_x2 and button_y1 <= self.cursor_y + 25 <= button_y2)

    
    def on_button_pressed(self, button):
        self.redraw_cursor("red")
        button.invoke()
        self.cursor_x = 290
        self.cursor_y = 275
        
        # Schedule changing back to original color after 2 seconds
        self.root.after(100, lambda: self.redraw_cursor("blue"))

    def redraw_cursor(self, color):
        ...
        x2 = x1 + cell_size
        y2 = y1 + cell_size
        self.cursor = self.create_cursor_label(x1, y1, x2, y2, bg=color)

    def create_cursor_label(self, x1, y1, x2, y2, **kwargs):
        cursor = tk.Label(self.root, **kwargs)
        cursor.place(x=x1, y=y1, width=x2-x1, height=y2-y1)
        return cursor
